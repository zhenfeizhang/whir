...
pub fn ntt<F: FFTField + 'static>(values: &mut [F]) {
    NttEngine::<F>::new_from_cache().ntt(values);
}

/// Compute the many NTTs of size `size` using a cached engine.
pub fn ntt_batch<F: FFTField + 'static>(values: &mut [F], size: usize) {
...
pub fn intt<F: FFTField + 'static>(values: &mut [F]) {
...
pub fn intt_batch<F: FFTField + 'static>(values: &mut [F], size: usize) {
    NttEngine::<F>::new_from_cache().intt_batch(values, size);
}

impl<F: FFTField + 'static> NttEngine<F> {
    ...
    pub(crate) fn new_from_fftfield() -> Self {
        ...
        if F::TWO_ADICITY <= 63 {
            Self::new(1 << F::TWO_ADICITY, F::root_of_unity())
        } else {
...
impl<F: Field> NttEngine<F> {
    pub fn new(order: usize, omega_order: F) -> Self {
        ...
        assert_eq!(omega_order.exp((order as u64).into()), F::ONE);
        assert_ne!(omega_order.exp((order as u64 / 2).into()), F::ONE);
        let mut res = Self {
            order,
            omega_order,
        ...
        if order % 3 == 0 {
            ...
            res.half_omega_3_1_min_2 = (omega_3_1 - omega_3_2) * F::INV_2;
            res.half_omega_3_1_plus_2 = (omega_3_1 + omega_3_2) * F::INV_2;
        }
        if order % 4 == 0 {
        ...
        if order % 8 == 0 {
            ...
            res.omega_8_3 = res.omega_8_1.exp(3u128);
        }
        if order % 16 == 0 {
            res.omega_16_1 = res.root(16);
            res.omega_16_3 = res.omega_16_1.exp(3u128);
            res.omega_16_9 = res.omega_16_1.exp(9u128);
        }
        res
    }
    ...
    pub fn root(&self, order: usize) -> F {
        ...
        self.omega_order.exp((self.order / order) as u128)
    }
...
mod tests {
    ...
    fn test_new_from_fftfield_basic() {
        ...
        let expected_root = Goldilocks::root_of_unity();
        let computed_root = engine.root(engine.order);
        assert_eq!(computed_root.exp(engine.order as u128), Goldilocks::ONE);
    ...
    fn test_root_computation() {
        ...
        assert_eq!(engine.root(8).exp(8u128), Goldilocks::ONE);
        assert_eq!(engine.root(4).exp(4u128), Goldilocks::ONE);
        assert_eq!(engine.root(2).exp(2u128), Goldilocks::ONE);

        // Ensure it's not a lower-order root
        assert_ne!(engine.root(8).exp(4u128), Goldilocks::ONE);
        assert_ne!(engine.root(4).exp(2u128), Goldilocks::ONE);
    }
    ...
    fn test_root_of_unity_multiplication() {
        ...
        assert_eq!(root.exp(2u128), engine.root(8));
        assert_eq!(root.exp(4u128), engine.root(4));
        assert_eq!(root.exp(8u128), engine.root(2));
    }
    ...
    fn test_root_of_unity_inversion() {
        ...
        // The inverse of ω is ω^{-1}, computed as ω^(p-2) in Goldilocks.
        let p: u64 = Goldilocks::MODULUS.as_u64();
        let inverse_root = root.exp(p as u128 - 2);
        assert_eq!(root * inverse_root, Goldilocks::ONE);
    }
    ...
    fn test_precomputed_small_roots() {
        ...
        assert_eq!(engine.omega_8_1.exp(8u128), Goldilocks::ONE);
        assert_eq!(engine.omega_8_3.exp(8u128), Goldilocks::ONE);
        assert_eq!(engine.omega_16_1.exp(16u128), Goldilocks::ONE);
        assert_eq!(engine.omega_16_3.exp(16u128), Goldilocks::ONE);
        assert_eq!(engine.omega_16_9.exp(16u128), Goldilocks::ONE);
    }

    #[test]
    fn test_consistency_across_multiple_instances() {
        let engine1 = NttEngine::<Goldilocks>::new_from_fftfield();
        let engine2 = NttEngine::<Goldilocks>::new_from_fftfield();
    ...
    fn test_roots_table_basic() {
        ...
        assert_eq!(roots_4[2], engine.root(4).exp(2u128));
        assert_eq!(roots_4[3], engine.root(4).exp(3u128));
    }
    ...
    fn test_roots_table_minimal_order() {
        ...
        assert_eq!(roots_2[0], Goldilocks::ONE);
    ...
    fn test_roots_table_progression() {
        ...
        for i in 0..4 {
            assert_eq!(roots_4[i], engine.root(4).exp(i as u128));
        }
    }
    ...
    fn test_apply_twiddles_basic() {
        ...
        let mut values = vec![
            ...
            Goldilocks::from(6u32),
            Goldilocks::from(7u32),
            Goldilocks::from(8u32),
        ];

        // Mock roots
        let r1 = Goldilocks::from(33u32);
        let roots = vec![r1];
        ...
        assert_eq!(values[1], Goldilocks::from(2u32));
        assert_eq!(values[2], Goldilocks::from(3u32));
        assert_eq!(values[3], Goldilocks::from(4u32));

        // The second row should be multiplied by the correct twiddle factors
        assert_eq!(values[4], Goldilocks::from(5u32)); // No change for first column
        assert_eq!(values[5], Goldilocks::from(6u32) * r1);
        assert_eq!(values[6], Goldilocks::from(7u32) * r1);
        assert_eq!(values[7], Goldilocks::from(8u32) * r1);
    }

    #[test]
    fn test_apply_twiddles_single_row() {
        let mut values = vec![Goldilocks::from(1u32), Goldilocks::from(2u32)];

        // Mock roots
        let r1 = Goldilocks::from(12u32);
        let roots = vec![r1];
    ...
    fn test_apply_twiddles_varying_rows() {
        let mut values = vec![
            ...
            Goldilocks::from(7u32),
            Goldilocks::from(8u32),
            Goldilocks::from(9u32),
        ];

        // Mock roots
        let roots = (2u32..100).map(Goldilocks::from).collect::<Vec<_>>();
        ...
        assert_eq!(values[3], Goldilocks::from(4u32));
        assert_eq!(values[4], Goldilocks::from(5u32) * roots[10]);
        assert_eq!(values[5], Goldilocks::from(6u32) * roots[20]);

        // Third row multiplied by twiddle factors
        assert_eq!(values[6], Goldilocks::from(7u32));
        assert_eq!(values[7], Goldilocks::from(8u32) * roots[20]);
        assert_eq!(values[8], Goldilocks::from(9u32) * roots[40]);
    }
    ...
    fn test_apply_twiddles_large_table() {
        ...
        let mut values: Vec<Goldilocks> = (0..size as u64).map(Goldilocks::from).collect();

        // Generate a large set of twiddle factors
        let roots: Vec<Goldilocks> = (0..(size * 2) as u64).map(Goldilocks::from).collect();
        ...
        for (i, &col) in values.iter().enumerate().take(cols) {
            assert_eq!(col, Goldilocks::from(i as u64));
        }
        ...
        for row in 1..rows {
            ...
            for col in 1..cols {
                ...
                let expected = Goldilocks::from(index as u64) * roots[idx];
    ...
    fn test_new_from_cache_singleton() {
        ...
        let engine1 = NttEngine::<Goldilocks>::new_from_cache();
        let engine2 = NttEngine::<Goldilocks>::new_from_cache();
    ...
    fn test_ntt_batch_size_2() {
        ...
        let f0 = Goldilocks::from(1u32);
        let f1 = Goldilocks::from(2u32);
    ...
    fn test_ntt_batch_size_4() {
        ...
        let f0 = Goldilocks::from(1u32);
        let f1 = Goldilocks::from(2u32);
        let f2 = Goldilocks::from(3u32);
        let f3 = Goldilocks::from(4u32);
    ...
    fn test_ntt_batch_size_8() {
        ...
        let f2 = Goldilocks::from(3u32);
        let f3 = Goldilocks::from(4u32);
        let f4 = Goldilocks::from(5u32);
        let f5 = Goldilocks::from(6u32);
        let f6 = Goldilocks::from(7u32);
        let f7 = Goldilocks::from(8u32);
    ...
    fn test_ntt_batch_size_16() {
        ...
        let values: Vec<_> = (1u32..=16).map(Goldilocks::from).collect();
        ...
        let mut expected_values = vec![Goldilocks::ZERO; 16];
        for (k, expected_value) in expected_values.iter_mut().enumerate().take(16) {
            let omega_k = omega.exp(k as u128);
            *expected_value = values
                .iter()
                .enumerate()
                .map(|(j, &f_j)| f_j * omega_k.exp(j as u128))
                .sum();
        }
    ...
    fn test_ntt_batch_size_32() {
        ...
        let values: Vec<_> = (1u32..=32).map(Goldilocks::from).collect();
        let mut values_ntt = values.clone();
        ...
        let mut expected_values = vec![Goldilocks::ZERO; 32];
